...
--- 1) List All Orders with Customer and Burger Names
SELECT co.order_id, co.customer_id, bn.burger_name, co.order_time
FROM customer_orders co
JOIN burger_names bn ON co.burger_id = bn.burger_id;


--- 2) Retrieve Orders with Runner Details
SELECT ro.order_id, br.runner_id, br.registration_date, ro.pickup_time
FROM runner_orders ro
JOIN burger_runner br ON ro.runner_id = br.runner_id;

--- 3) Fetch Orders That Were Cancelled and the Reason for Cancellation
SELECT ro.order_id, ro.cancellation
FROM runner_orders ro
WHERE ro.cancellation IS NOT NULL;

--- 4) Subquery to Find Runners Who Delivered Orders
SELECT runner_id, registration_date
FROM burger_runner
WHERE 
    runner_id IN (
        SELECT DISTINCT runner_id FROM runner_orders WHERE cancellation IS NULL
    );


--- 5)Calculate Subtotal for Each Order
SELECT co.order_id, co.customer_id, bn.burger_name, 
(100 + COALESCE(LEN(co.extras), 0) * 10 - COALESCE(LEN(co.exclusions), 0) * 5) AS subtotal
FROM customer_orders co
JOIN burger_names bn ON co.burger_id = bn.burger_id;


--- 6)Count Orders Grouped by Each Burger Type
...
--- 7)Find Runners Who Completed More Than One Delivery
SELECT ro.runner_id, COUNT(ro.order_id) AS completed_deliveries
FROM runner_orders ro
